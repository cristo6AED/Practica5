...
print("Número de observaciones completas")
sum(complete.cases(dirty_iris)==TRUE)

porcentaje_aciertos <- sum(complete.cases(dirty_iris)==TRUE)/length(complete.cases(dirty_iris))
print("Porcebtaje de observaciones completas")
porcentaje_aciertos
...
```{r , include = FALSE}
# Regla 3 sigma
reglasigma <- function(datos){
  mu <- mean(datos, na.rm = TRUE)       # media ignorando NA
  sigma <- sd(datos, na.rm = TRUE)      # desviación típica ignorando NA
  
  # Crear vector lógico: TRUE si es outlier, FALSE si no
  outliers <- abs(datos - mu) > 3 * sigma
  
  # Mantener NA en su posición original
  outliers[is.na(datos)] <- NA
...
reglahampel <- function(datos){
  mediana <- median(datos, na.rm=TRUE)
  dispersión <- madm(datos)
  
  outliers <- abs(datos - med) > 3 * mediana
...
reglaboxplot <- function(x) {
  # Calcular cuartiles ignorando NA
  ...
  # Detectar outliers
  outliers <- (x < (x_i - 1.5 * iqr)) | (x > (x_s + 1.5 * iqr))
...
reglapercentil <- function(x) {
  ...
  p <- quantile(x, probs = c(0.05, 0.95), na.rm = TRUE)
  lower <- p[1]
  upper <- p[2]
